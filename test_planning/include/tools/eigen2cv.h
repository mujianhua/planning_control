...
template <typename T> struct opencv_matrix;

template <> struct opencv_matrix<double> { static const int type = CV_64FC1; };
template <> struct opencv_matrix<float> { static const int type = CV_32FC1; };
template <> struct opencv_matrix<int> { static const int type = CV_32SC1; };
template <> struct opencv_matrix<unsigned char> {
    static const int type = CV_8UC1;
};
template <> struct opencv_matrix<bool> {
...
class Eigen2CVBase {
  ...
  protected:
    template <typename Derived> void mapPlaneMemory(const Derived &src) {
...
class Eigen2CV<Derived, Eigen::PlainObjectBase<Derived>, details::Mutable>
  ...
  public:
    ...
    template <typename T> Self &operator=(const cv::Mat_<T> &src) {
