...
void BaseSolver::SetHessianMatrix(Eigen::SparseMatrix<double> *matrix_h) const {
...
void BaseSolver::SetConstraintMatrix(
    ...
    for (int i = 0; i < n_ - 1; i++) {
        const auto ref_k = ref_points[i].path_point.kappa;
        const auto ds =
            ref_points[i + 1].path_point.s - ref_points[i].path_point.s;
        const auto ref_d_k = (ref_points[i + 1].path_point.kappa - ref_k) / ds;
        a(1, 0) = -pow(ref_k, 2);
    ...
    *upper_bound = Eigen::VectorXd::Zero(cons_size_, 1);
    ...
    x0 << init_error[0], init_error[1],
        vehicle_state_->getStartPoint().path_point.kappa;
    ...
    for (int i = 0; i < n_; i++) {
        (*lower_bound)(kappa_idx + i) =
            -tan(FLAGS_max_steering_angle) / FLAGS_wheel_base; // wuyu..........
    ...
    if (FLAGS_constraint_end_heading && reference_path_->IsBlocked()) {
        double end_psi = math::ConstrainAngle(
            vehicle_state_->getTargetPoint().path_point.theta -
            ref_points.back().path_point.theta);
...
BaseSolver::GetSoftBounds(double lb, double ub, double safety_margin) const {
...
void BaseSolver::GetOptimizedPath(
    ...
    for (int i = 0; i < n_; i++) {
        ...
        double angle = ref_points[i].path_point.theta;
        tmp_point.path_point.x = ref_points[i].path_point.x +
                                 optimization_result(3 * i) *
                                     cos(math::ConstrainAngle(angle + M_PI_2));
        tmp_point.path_point.y = ref_points[i].path_point.y +
                                 optimization_result(3 * i) *
                                     sin(math::ConstrainAngle(angle + M_PI_2));
        tmp_point.path_point.kappa = optimization_result(3 * i + 2);
        tmp_point.path_point.theta =
            math::ConstrainAngle(angle + optimization_result(3 * i + 1));
        if (i != 0) {
            tmp_s += sqrt(pow(tmp_point.path_point.x -
                                  optimized_path->back().path_point.x,
                              2) +
                          pow(tmp_point.path_point.y -
                                  optimized_path->back().path_point.y,
                              2));
        }

        tmp_point.path_point.s = tmp_s;
