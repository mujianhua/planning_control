...
TrajectoryNLPOptimizer::TrajectoryNLPOptimizer(const PlanningConfig &config)
    : config_(config) {
...
void TrajectoryNLPOptimizer::BuildIterativeNLP() {
...
double TrajectoryNLPOptimizer::SolveIteratively(
    double w_inf, const OptiConstraints &constraints, const OptiStates &guess,
    const DiscretizedTrajectory &reference, OptiStates &result) {
    auto identity = DM::ones(config_.nfe, 1);

    DM lb_x, lb_y, lb_theta, lb_v, lb_phi, lb_a, lb_omega, lb_jerk, lb_xf,
        lb_yf, lb_xr, lb_yr;
    DM ub_x, ub_y, ub_theta, ub_v, ub_phi, ub_a, ub_omega, ub_jerk, ub_xf,
        ub_yf, ub_xr, ub_yr;
    ...
    DMDict arg, res;
    arg["lbx"] = DM::vertcat({lb_x, lb_y, lb_theta, lb_v, lb_phi, lb_a,
                              lb_omega, lb_jerk, lb_xf, lb_yf, lb_xr, lb_yr});
    arg["ubx"] = DM::vertcat({ub_x, ub_y, ub_theta, ub_v, ub_phi, ub_a,
                              ub_omega, ub_jerk, ub_xf, ub_yf, ub_xr, ub_yr});
    arg["x0"] = DM::vertcat({guess.x, guess.y, guess.theta, guess.v, guess.phi,
                             guess.a, guess.omega, guess.jerk, guess.xf,
                             guess.yf, guess.xr, guess.yr});

    DM ref_x(config_.nfe, 1), ref_y(config_.nfe, 1), ref_theta(config_.nfe, 1);
    for (int i = 0; i < config_.nfe; i++) {
        ref_x(i) = reference.trajectory_points()->at(i).x;
        ref_y(i) = reference.trajectory_points()->at(i).y;
        ref_theta(i) = reference.trajectory_points()->at(i).theta;
    }
