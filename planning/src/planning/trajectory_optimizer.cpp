...
void TrajectoryOptimizer::CalculateInitialGuess(States &states) const {
    ...
    for (size_t i = 1; i < states.x.size(); i++) {
        ...
        states.omega[i] =
            std::min(vehicle_.omega_max,
                     std::max(-vehicle_.omega_max,
...
bool TrajectoryOptimizer::FormulateCorridorConstraints(
    ...
    for (size_t i = 0; i < config_.nfe; i++) {
        ...
        std::tie(states.xf[i], states.yf[i], states.xr[i], states.yr[i]) =
            vehicle_.GetDiscPositions(states.x[i], states.y[i],
                                      states.theta[i]);

        math::AABox2d box;
        if (!GenerateBox(time, states.xf[i], states.yf[i], vehicle_.radius,
                         box)) {
            return false;
        }
        ...
        if (!GenerateBox(time, states.xr[i], states.yr[i], vehicle_.radius,
                         box)) {
            return false;
        }
