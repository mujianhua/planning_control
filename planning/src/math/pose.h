...
class Pose {
 public:
  ...
  inline Pose relativeTo(const Pose &coord) const {
    ...
    return {dx * cos(coord.theta()) + dy * sin(coord.theta()),
            -dx * sin(coord.theta()) + dy * cos(coord.theta()),
            theta_ - coord.theta()};
  }

  inline Pose extend(double length) const { return transform({length, 0, 0}); }

  inline Pose transform(const Pose &relative) const {
    return {x_ + relative.x() * cos(theta_) - relative.y() * sin(theta_),
