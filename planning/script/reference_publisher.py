...
def get_random_reference_points(center_line, count, start_idx=100, back_idx=500):
    random_stations = np.random.randint(
        start_idx, len(center_line.points) - back_idx, count)
...
def generate_random_vehicles(center_line, count, length=4.0, width=2.0):
    ...
    random_laterals = lateral_samples[np.random.randint(
        0, len(lateral_samples), count)]
    ...
    for i in range(count):
        footprint = transform_footprint(
            ob_x[i], ob_y[i], ref_theta[i], [length, width])
        polygon = Polygon(points=[Point32(x=row[0], y=row[1])
...
def generate_random_dynamic_vehicles(center_line, count, horizon=16.0, dt=0.1):
    ...
    for i in range(count):
        ...
        traj_x, traj_y = convert_frenet_to_cartesian(
            ref_center_line[s_ind, :], np.repeat(rand_lateral, traj_len))
...
def main():
    ...
    if 'dynamic' in sys.argv or 'pedestrian' in sys.argv:
        ...
        if 'pedestrian' in sys.argv:
            dynamic_obstacles.obstacles.extend(
                generate_random_pedestrian(center_line, 6).obstacles)

        if 'dynamic' in sys.argv:
            dynamic_obstacles.obstacles.extend(
                generate_random_dynamic_vehicles(center_line, 3).obstacles)

        dynamic_obstacles_pub = rospy.Publisher(
    ...
    if 'serialize' in sys.argv:
        pickle_path = os.path.join(
            os.path.dirname(__file__), 'reference.pickle')
        with open(pickle_path, 'w') as f:
            pickle.dump({"center": center_line, "static": static_obstacles,
                        "dynamic": dynamic_obstacles}, f)
